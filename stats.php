...
<div id="menu">
  ...
  <div class="middle">
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="0" class="oneview">
		<tr valign="top">
			<?php
			$sql = "SELECT SUM(user), SUM(view) FROM days";
			try {
				...
				if ($data) {
					...
					echo "No data found in the 'days' table.";
				}
				$stmt = null;
		...
		<tr valign="top">
			...
			$abfrage = $conn->prepare("SELECT COUNT(visitor_id) FROM visitors WHERE online >= :isonline");
		...
		<tr valign="top">
			<?php
			// Bounce
			$abfrage = $conn->prepare("SELECT COUNT(visitor_id) FROM visitors");
			$abfrage->execute();
			$total = $abfrage->fetchColumn();

			$abfrage = $conn->prepare("SELECT COUNT(visitor_id) FROM visitors WHERE online = time");
			$abfrage->execute();
		...
		<tr valign="top">
			...
			$abfrage = $conn->prepare("SELECT SUM(user) FROM days WHERE day = :sel_tag");
			...
			$abfrage = $conn->prepare("SELECT COUNT(visitor_id) FROM visitors WHERE time >= :anfangTag AND time <= :endeTag");
		...
		<table height="200" width="100%" cellpadding="0" cellspacing="0" align="right">
			<tr valign="bottom" height="180">
				...
				for($Stunde=23; $Stunde>=0; $Stunde--) {
					...
					$abfrage = $conn->prepare("SELECT COUNT(visitor_id) FROM visitors WHERE time >= :anfangStunde AND time <= :endeStunde");
